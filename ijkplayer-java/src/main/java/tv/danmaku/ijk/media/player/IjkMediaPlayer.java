    ...
    private static boolean onNativeInvoke(Object weakThiz, int what, Bundle args) {
        ...
        switch (what) {
            case OnNativeInvokeListener.CTRL_WILL_CONCAT_RESOLVE_SEGMENT: {
                OnControlMessageListener
                    onControlMessageListener
                    = player.mOnControlMessageListener;
            ...
            case OnNativeInvokeListener.EVENT_DID_HTTP_SEEK: {
                if (args != null && (args.getInt("error") == -5 || args.getInt("error") == -101)) {
                    if (player.mEventHandler != null) {
                        Message m = player.mEventHandler.obtainMessage(MEDIA_ERROR,
                                                                       MEDIA_ERROR_IO, 0);
